...
vec3 color(vec2 coords,float blur) //processing the sample
{
        ...
        col.r = texture2D(chainedTexture,coords + vec2(0.0,1.0)*texelSize*fringe*blur).r;
        col.g = texture2D(chainedTexture,coords + vec2(-0.866,-0.5)*texelSize*fringe*blur).g;
        col.b = texture2D(chainedTexture,coords + vec2(0.866,-0.5)*texelSize*fringe*blur).b;
...
{
        float noiseX = ((fract(1.0-coord.s*(screenDimentions.x/2.0))*0.25)+(fract(coord.t*(screenDimentions.y/2.0))*0.75))*2.0-1.0;
        float noiseY = ((fract(1.0-coord.s*(screenDimentions.x/2.0))*0.75)+(fract(coord.t*(screenDimentions.y/2.0))*0.25))*2.0-1.0;
...
void main()
{
        ...
        float depth = linearize(texture2D(depthTexture,texCoord.xy).x);
        ...
        else
        {
                ...
                float d = fDepth/125.0; //focal plane in mm
                float o = depth/125.0; //depth in mm
        ...
        float w = (1.0/screenDimentions.x)*blur*maxblur+noise.x;
        float h = (1.0/screenDimentions.y)*blur*maxblur+noise.y;
        ...
        {
                col = texture2D(chainedTexture, texCoord.xy).rgb;
        }
